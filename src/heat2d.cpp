...
//typedef typename std::chrono::time_point<std::chrono::system_clock> clock;  

//you can template the function to use
//in other dimensions

//needs to be made a non member function
    ...
    alpha(in_alpha), m(in_m), dt(in_dt), M(in_m * in_m)
{
    ...
    double expr;
    //std::cout << "r,c" << rows << " " << columns << std::endl;
    for (auto i = 0; i < M.get_rows(); i++)
    {
        for(auto j = i; j < M.get_columns(); j++)
        {
            ...
            //std::cout << "laplace <0> :" << laplace_op_2d<0>(i,j, this->m) << std::endl;
            //std::cout << "laplace <1> :" << laplace_op_2d<1>(i,j, this->m) << std::endl;
            //std::cout << laplace_op_2d<0>(i,j, this->m) << std::endl;
            expr = ones - (alpha * (dt/dxsquared) * (laplace_op_2d<0>(i,j, this->m)+ laplace_op_2d<1>(i,j, this->m)));
    ...
    std::cout << "time elapsed for making the matrix is : " << float(std::clock()- begin_time) << std::endl; 
...
Vector<double> Heat2D::solve(double time_end) const
{
    ...
    for(auto i = 0; i < time_end/dt ; i++)
    {
        ...
        //std::cout << "Copying vectors" << std::endl;
